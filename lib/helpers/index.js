    ...
    value: function buildUrl(event, type, isCrappyIE) {
      ...
      switch (type) {
        ...
        default:
          ...
          calendarUrl = ["BEGIN:VCALENDAR", "VERSION:2.0", "BEGIN:VEVENT", "URL:" + document.URL, "DTSTART:" + !useDateTime ? this.formatStartDate(event.startTime) : this.formatTime(event.startTime), "DTEND:" + !useDateTime ? this.formatEndDate(event.endTime) : this.formatTime(event.endTime), "SUMMARY:" + event.title, "DESCRIPTION:" + formattedDescription, "LOCATION:" + event.location, "END:VEVENT", "END:VCALENDAR"].join("\n");
